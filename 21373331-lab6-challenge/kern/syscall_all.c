...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm)
{
	...
	if (srcva != 0)
	{
		...
		int r = page_insert(e->env_pgdir, e->env_asid, p, e->env_ipc_dstva, perm);
		if (r)
		{
			return r;
		}
...
int sys_get_relative_path(char *dst)
{   
	if (dst == 0)
	{
		return -1;
...
int sys_set_relative_path(char *path)
{
	if (strlen(path) >= 1024)
	{
		return -1;
	}
	strcpy(curenv->r_path, path);
	return 0;
}
